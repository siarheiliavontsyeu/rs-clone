<template>
  <v-sheet class="container-slider mx-auto" elevation="8">
    <h2><slot></slot></h2>
    <v-slide-group
      v-if="movies.isItemsReady"
      class="pa-1"
      selected-class="bg-success"
      show-arrows
    >
      <v-slide-group-item v-for="movie in movies.array" :key="movie.filmId">
        <v-card
          @click="
            $router.push({
              name: 'movie',
    ...
    <v-slide-group v-else class="pa-1" selected-class="bg-success" show-arrows>
      <v-slide-group-item
        v-for="n in 10"
        :key="n"
