...
const props = defineProps<{
  // review: Omit<ReviewResponseI, 'positiveRating'|'negativeRating'>
...
const date = computed(() => {
  ...
  const hours = date_.getHours() <10 ? `0${date_.getHours()}` : date_.getHours();
  const minutes = date_.getMinutes() < 10 ? `0${date_.getMinutes()}` : date_.getMinutes();
